...
def _join_background_init_threads(timeout: float = 3.0) -> None:
    """Wait for any 'detector-pipeline-init' thread(s) started by load() to
    fully finish (build + warmup), so a slow-to-settle background thread
    never outlives the test's ctx-patching fixture and trips over the real
    (unpatched) mitmproxy.ctx in a later test."""
    for t in threading.enumerate():
        if t.name == "detector-pipeline-init":
