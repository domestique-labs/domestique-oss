# OpenAI Codex CLI → Domestique

**Status:** ⚠️ Known gap — Codex uses the OpenAI **Responses API** (`/v1/responses`),
which Domestique does **not** yet intercept. Prompts sent that way reach OpenAI
**unredacted.**

## What works today

Domestique handles the OpenAI **Chat Completions** family
...
Codex defaults to the Responses API. Domestique's proxy passes any unrecognized path —
including `/v1/responses` — **straight through untouched**, so redaction is not applied.
Don't rely on Domestique to sanitize a Codex session until this row turns ✅.

Tracking: a `/v1/responses` handler is the next transport-layer task (Lane 3). Until it
lands, treat Codex as **unprotected** through Domestique.
