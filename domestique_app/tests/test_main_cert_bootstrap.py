...
class TestEnsureCertGeneratedPortable:
    def test_generates_ca_when_not_setup(self):
        ...
        with (
            ...
            patch("domestique_app.services.cert_manager.is_cert_trusted", return_value=True),
        ):
    ...
    def test_trusts_ca_when_not_yet_trusted(self):
        ...
        with (
            ...
            patch(
                "domestique_app.services.cert_manager.install_and_trust", return_value=True
            ) as mock_install,
        ):
            main._ensure_cert_generated_portable()
    ...
    def test_does_not_reinstall_when_already_trusted(self):
        ...
        with (
            patch("domestique_app.server.api.get_browser_proxy_service", return_value=svc),
            patch("domestique_app.services.cert_manager.is_cert_trusted", return_value=True),
            patch("domestique_app.services.cert_manager.install_and_trust") as mock_install,
        ):
            main._ensure_cert_generated_portable()
    ...
    def test_trust_returning_false_is_non_fatal_and_logged(self, capsys):
        ...
        with (
            patch("domestique_app.server.api.get_browser_proxy_service", return_value=svc),
            patch("domestique_app.services.cert_manager.is_cert_trusted", return_value=False),
            patch("domestique_app.services.cert_manager.install_and_trust", return_value=False),
        ):
            main._ensure_cert_generated_portable()  # must not raise
    ...
    def test_setup_failure_is_non_fatal_and_skips_trust(self):
        ...
        with (
            patch(
                "domestique_app.server.api.get_browser_proxy_service",
