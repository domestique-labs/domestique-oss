...
from domestique_app.services.mitm_addon import (
...
class TestCAGeneration:
    ...
    def test_generate_ca_creates_files(self, tmp_path):
        ...
        with (
            patch("domestique_app.services.interceptor.CA_DIR", tmp_path),
            patch("domestique_app.services.interceptor.CA_KEY_PATH", key_path),
            patch("domestique_app.services.interceptor.CA_CERT_PATH", cert_path),
        ):
...
class TestPACFile:
    ...
    def test_generate_pac_file(self, tmp_path):
        ...
        with patch("domestique_app.services.interceptor.PAC_PATH", pac_path):
...
class TestDomainManagement:
    ...
    def test_add_custom_domain(self):
        with patch("domestique_app.services.interceptor.generate_pac_file"):
...
class TestBrowserProxyService:
    ...
    def test_start_requires_ca(self, mock_popen):
        svc = BrowserProxyService()
        with patch("domestique_app.services.interceptor.CA_CERT_PATH") as mock_path:
    ...
    def test_setup_generates_ca(self, tmp_path):
        ...
        with (
            patch("domestique_app.services.interceptor.CA_DIR", tmp_path),
            patch("domestique_app.services.interceptor.CA_KEY_PATH", tmp_path / "ca.key"),
            patch("domestique_app.services.interceptor.CA_CERT_PATH", tmp_path / "ca.pem"),
            patch("domestique_app.services.interceptor.PAC_PATH", tmp_path / "proxy.pac"),
            patch("domestique_app.services.interceptor.is_ca_installed", return_value=True),
        ):
            results = svc.setup()
...
class TestWindowsSystemProxyPACOnly:
    ...
    def _patch_platform(self, monkeypatch, tmp_path):
        monkeypatch.setattr("domestique_app.services.interceptor.is_windows", lambda: True)
        monkeypatch.setattr("domestique_app.services.interceptor.is_macos", lambda: False)
        monkeypatch.setattr(
            "domestique_app.services.interceptor.WINDOWS_PROXY_BACKUP_PATH",
            tmp_path / "windows_proxy_backup.json",
        )
        monkeypatch.setattr("domestique_app.services.interceptor.PAC_PATH", tmp_path / "proxy.pac")
...
class TestMacOSSystemProxyPACOnly:
    ...
    def _patch_platform(self, monkeypatch):
        monkeypatch.setattr("domestique_app.services.interceptor.is_windows", lambda: False)
        monkeypatch.setattr("domestique_app.services.interceptor.is_macos", lambda: True)
        monkeypatch.setattr(
            "domestique_app.services.interceptor._get_all_active_interfaces", lambda: ["Wi-Fi"]
        )
    ...
    def test_enable_sets_autoproxy_only_no_blanket_webproxy(self, tmp_path, monkeypatch):
        ...
        with patch("domestique_app.services.interceptor.subprocess.run", side_effect=fake_run):
            result = enable_system_proxy(port=8080)
